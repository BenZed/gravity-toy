...
var SimulationCanvasDraw = function () {
  function SimulationCanvasDraw(simulation, canvas) {
    ...
    this[_drawBody] = function (body) {
      ...
      //at faster than 1x
      var timeDialation = Math.min(Math.abs(_this.tickDelta), MAX_TIME_DIALATION);
      ...
      _this.context.ellipse(pos.x, pos.y, Math.max(radius, MIN_DRAW_RADIUS), Math.max(blurRadius, MIN_DRAW_RADIUS), angle, 0, 2 * Math.PI);
    ...
    this[_drawTrails] = function (body, back) {
      ...
      var scale = Math.max(_this.camera[_current].scale, 1);

      var step = Math.floor(_this.options.trailStep * scale);
      ...
      while (length > 0) {
        ...
        _this.context.strokeStyle = 'rgba(' + style + ',' + length / TRAIL_FADE_START + ')';
      }
    ...
    this[_drawComplete] = function (deltaTime) {
      ...
      _this.simulation.forEachBody(function (body) {
        if (body.trails) _this[_drawTrails](body, true);

        if (body.predictions) _this[_drawTrails](body, false);
      });
      ...
      if (nextTick !== _this.tick + _this.tickDelta) _this.tickDelta = (0, _sign2.default)(_this.tickDelta);
