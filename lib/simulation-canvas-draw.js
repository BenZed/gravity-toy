...
var SimulationCanvasDraw = function () {
  ...
  (0, _createClass3.default)(SimulationCanvasDraw, [{
    ...
    value: function value(body) {
      ...
      var vel = stats.vel.sub(this.camera.speed).div(current.scale).mult(0.001 * this.simulation.UPDATE_DELTA * 0.5);

      //speedRadius will warp the body from a circle to an ellipse if it is moving fasting enough
      var speedRadius = vel.magnitude;
      ...
      var opacity = (0, _lerp2.default)(0.5, 1, radius / speedRadius);
      ...
      this.context.ellipse(pos.x, pos.y, Math.max(radius, MIN_DRAW_RADIUS), Math.max(speedRadius, MIN_DRAW_RADIUS), angle, 0, 2 * Math.PI);
