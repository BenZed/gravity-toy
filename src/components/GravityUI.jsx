...
export default class GravityUI extends React.Component {
  ...
  constructor(props) {
    ...
    this.simulation.on('body-create', (body) => {
      const name = Math.random().toString(36).substring(2,8).toUpperCase()
  ...
  createNebula(radius, offset, turbulence) {
    ...
    while (total < radius * 1) {
      ...
      const cloudOffset = Vector.randomInCircle(radius)
      const cloudRadius = Math.random() * (radius * 0.5)
      const count = cloudRadius * 0.2

      for (let i = 0; i < count; i++) {
        const mass = Math.random() * 10000 + 100
  ...
  createProtoDisc(radius, offset, density) {
    ...
    while (total < radius * density) {
      ...
      const vel = relPos.perpendicular().normalized().mult(speed)
  ...
  createInitialBodies() {
    const windowCenter = new Vector(canvas.width * 0.5, canvas.height * 0.5)
